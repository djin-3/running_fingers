...
- [ ] タップエフェクト
  ...
  - [ ] 無効タップ: 赤色フラッシュ + シェイク
  - [ ] 速度連動エフェクト（5段階、直近10タップ平均tpsで計算）
    - [ ] Lv1〜5のデフォルト演出（白/薄青の光のみ）
    - [ ] 速度低下時の即時レベルダウン
  - [ ] 課金スキン対応（色テーマ差し替え構造）
- [ ] スタート演出
  - [ ] "On your mark" → "Set" → "Go!!" テキストアニメーション
  - [ ] カウントダウン時の緊張感ある演出
- [ ] リザルト演出
...
次にやること（Phase 4）:
1. タップエフェクト（速度連動5段階、デフォルトは白/薄青の光、課金スキンで色テーマ差し替え）
2. スタート演出（テキストアニメーション）
3. リザルト演出（自己ベスト更新時のエフェクト）
4. ボタン位置調整（待機画面でドラッグ移動、左右独立、設定保存）
5. 効果音（未定・後回し可）
6. モバイル最適化（画面スリープ防止・バックグラウンド中断・画面サイズ対応）
